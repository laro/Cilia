...
> The use of plain local/stack-allocated variables or of 'hierarchically managed' member variables is still preferred, of course. Use (smart) pointers only when you need them.
...
For shared pointers using `makeShared` is more efficient (due to the single-allocation optimization).

```
ContactInfo^ sharedPointerToContactInfo = makeShared<ContactInfo>()
```
...
A "raw", plain, classical C/C++ pointer. In Cilia, a `T*` typically is without ownership, as for pointers _with_ ownership you would use a `T+` pointer instead. In C++, ownership depends, case by case. 
...
`T[0]+`/`T[0]^` is a unique/shared pointer to a C/C++ _array_ of fixed size, but a size that is unknown to the type system (`0` is just a dummy here).
