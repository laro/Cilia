...
- The loop variable of `for ... in` is **by default passed as `in`**.
     ...
     `using String::InArgumentType = const StringView`
        ...
        - Example:
            ...
            - **`String[] stringArray = ["a", "b", "c"]`**  
              **`for str in stringArray { ... }`**
                - `str` is `const StringView`
    - Not every view type (because the views typically do not guarantee contiguous memory access as they do support stride):
        - ~~`Matrix` - `MatrixView`~~
        - ~~`Image` - `ImageView`~~
...
- Type trait **`CopyArgumentType`**
    ...
    - Example:
        - `for copy str in ["an", "array", "of", "words"] { ... }`
            ...
            - This way people do not necessarily need to understand the concept of a `StringView` literal. They simply write `copy` to get a `String` with a copy of the `StringView`.
