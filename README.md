...
- `Type^ instance`, inspired by C++/CLI.
    ...
    - Possible to redefine for interoperability with other languages:
        ...
        - C#/.NET classes: Use garbage collected memory, add pointers to the global list of C#/.NET instance pointers (with GCHandle and/or gcroot). Access/dereferencing  creates a temporary `DotNetGCPinnedPtr`, that pins the object (so the Gargabe Collector cannot move it during access).  
