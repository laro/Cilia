...
- Type traits `InArgumentType` to determine the concrete type to be used for `in`-passing.
    ...
    - Special trick for **types with views**, e.g. `String`/`StringView`:  
      ...
      so _all_ functions with an `in String` parameter would implicitly accept a `String` and _also_ a `StringView`. So people do not necessarily need to understand the concept of a `StringView`, they simply write `String`. And there is no need to define two functions (one for `String` and one for `StringView`) anymore.  
