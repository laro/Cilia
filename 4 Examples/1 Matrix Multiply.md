...
Triple nested loop, O(n³). For each element `C[i,j]`, compute the dot product of row `i` of `A` and column `j` of `B`. Using the transposed of `A` for a cache-friendly access pattern, but no tiling.
